...
public class PlayerInteraction
{
    ...
    public void StartHoldInteract()
    {
        ...
        if (Physics.Raycast(ray, out hit, interactDistance, interactLayer))
        {
            ...
            // TeleportDoor теперь работает автоматически при входе в триггер, не требует зажатия клавиши
            // Проверяем, является ли объект телепорт-дверью (только для информации)
            TeleportDoor teleportDoor = hit.collider.GetComponent<TeleportDoor>();
            if (teleportDoor != null)
            {
                Debug.Log("ℹ Найдена телепорт-дверь. Телепортация произойдет автоматически при входе в триггер.");
                // Не начинаем зажатие - TeleportDoor работает автоматически через OnTriggerEnter
                return;
            }
        ...
        // TeleportZone теперь работает автоматически при входе в триггер, не требует зажатия клавиши
        // Проверяем, находимся ли мы в зоне телепортации (только для информации, не для зажатия)
        TeleportZone teleportZone = GetNearbyTeleportZone();
        if (teleportZone != null)
        {
            Debug.Log("ℹ Найдена зона телепортации. Телепортация произойдет автоматически при входе в триггер.");
            // Не начинаем зажатие - TeleportZone работает автоматически через OnTriggerEnter
        }
        else
        {
    ...
    private void TryPurchaseItem(GameObject itemObject)
    {
        ...
        // Сохраняем информацию о предмете для респавна
        int itemSlotIndex = shopZone.GetItemSlotIndex(itemObject);
        bool isShopItem = itemSlotIndex >= 0;
        ...
        // Респавним предмет в магазине, если это предмет из магазина
        if (isShopItem)
        {
            // Респавним новый случайный предмет в том же слоте
            shopZone.RespawnItemAtSlot(itemSlotIndex);
            Debug.Log($"🔄 Респавнен предмет в магазине в слоте {itemSlotIndex}");
        }
        else
        {
            // Уничтожаем предмет только если это не предмет из магазина
            // (оружие уже в инвентаре, его не нужно уничтожать)
