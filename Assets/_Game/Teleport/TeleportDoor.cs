...
// Общий класс для управления кулдауном телепортации
public static class TeleportCooldownManager
{
    private static float lastTeleportTime = -999f;
    private static float cooldownDuration = 2f;

    public static void SetCooldown(float duration)
    {
    ...
    public static float GetRemainingCooldown()
    {
        ...
        return Mathf.Max(0f, cooldownDuration - timeSinceLastTeleport);
    }

    public static void RegisterTeleport()
    {
...
public class TeleportDoor : MonoBehaviour
{
    ...
    [Header("World Sign")]
    [Tooltip("Показывать подпись над дверью")]
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (IsPlayer(other) && !isTeleporting)
        {
            ...
            if (!TeleportCooldownManager.CanTeleport())
            {
                ...
                Debug.Log($"⏳ TeleportDoor: Кулдаун активен. Осталось {remainingCooldown:F1} секунд");
                return;
            }
